<div class="content-holder">
    <div>
        <h2>CUSTOMERS CHECK-IN</h2>
    </div>
    <body>
        <div class="table-wrapper">
             <div class="table-header">ACTIVE CHECK-INS: FUNCTION HALLS</div>
            <div class="table-scroll">
                <table class="reservationTable">
                    ...
                    <tbody>
                       ...
                       $customerList = mysqli_query($db, "SELECT * FROM `check_in_out_master` WHERE unit_type = 'Function Hall' AND status = 'Checked-In'");
                        $counter = 0;
                            ...
                            <td>
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    <button type="button" class="btn btn-secondary" 
                                        onclick="checkOut('<?php echo $row['reservation_id']; ?>', '<?php echo $row['unit_id']; ?>')">
                                        Check-Out
                                    </button>
                                   <button type="button" class="btn btn-secondary" 
                                    onclick="viewCustomerDetails('<?php echo $row['customer_id']; ?>', 'checkin')">View Details</button>
                            </td>
                        </tr>
                        <?php
                        }
